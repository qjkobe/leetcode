...
//虽然Test<T1,T2>，但类名字仍为Test。T1,T2只是告诉我们new的时候要加入泛型：声明，更方便通用
//T1,T2可以任意定义，比如改成K,V都可以
//他们不会关联到其他类，只是在本类中通用
public class GenericsDemo<T1, T2> {

  public static void main(String[] args) {
    System.out.println(new GenericsDemo().getT1());//T1
    System.out.println(new GenericsDemo().getT2());//T2
    new GenericsDemo<String, String>().getClassName(1);// T1,T2在new的时候用到，这里都为String
    Class classAndList = new GenericsDemo().getClassAndList(Integer.class);
    System.out.println(classAndList); //class java.lang.Integer
    // HashMap的括号里面不能是T,E,T1,T2等不确定的东西,但可以使用?
